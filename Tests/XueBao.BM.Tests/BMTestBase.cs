...
namespace XueBao.BM.Tests
{
    public abstract class BMTestBase : AbpIntegratedTestBase<BMTestModule>
    {
        ...
        protected void UsingDbContext(Action<BMDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<BMDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BMDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BMDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BMDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BMDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<BMDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BMDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BMDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BMDbContext, Task<T>> func)
        {
            T result;
